...
{					   
	...
	else    /* 9341/6804/5310/1963/7789/9481  等发送开启显示指令 */
...
void LCD_Scan_Dir(u8 dir)
{
	...
	u16 dirreg=0;
	u16 temp;  
	if((lcddev.dir==1&&lcddev.id!=0X6804&&lcddev.id!=0X1963&&lcddev.id!=0x9481)||(lcddev.dir==0&&lcddev.id==0X1963))//横屏时，对6804和1963不改变扫描方向！竖屏时1963改变方向
	{			   
		switch(dir)//方向转换
		{
			case 0:dir=6;break;
	...
	if(lcddev.id==0x9341||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963||lcddev.id==0x9481||lcddev.id==0x7789||lcddev.id==0x9486||lcddev.id==0x9488||lcddev.id==0x7796)//9341/6804/5310/5510/1963,特殊处理
	{
		switch(dir)
		{
		...
		if(lcddev.id==0X6804 ||lcddev.id==0x9481)regval|=0x02;//6804的BIT6和9341的反了	   
		...
		if(lcddev.id!=0X1963)//1963不做坐标处理
		{
			...
			}else if(lcddev.id!=0x9481||lcddev.dir!=1)  //lish84
			{
				if(lcddev.width>lcddev.height)//交换X,Y
				{
					temp=lcddev.width;
...
void LCD_Display_Dir(u8 dir)
{
	if(dir==0)			//竖屏
	{
		...
		lcddev.width=240;
		lcddev.height=320;
		if(lcddev.id==0X9341||lcddev.id==0X6804||lcddev.id==0X5310 ||lcddev.id==0x9481||lcddev.id==0x7789||lcddev.id==0x9486||lcddev.id==0x9488||lcddev.id==0x7796)
		{
			lcddev.wramcmd=0X2C;
	 		lcddev.setxcmd=0X2A;
			lcddev.setycmd=0X2B;  	 
			if(lcddev.id==0X6804||lcddev.id==0X5310 ||lcddev.id==0x9481||lcddev.id==0x9486||lcddev.id==0x9488||lcddev.id==0x7796)
			{
				lcddev.width=320;
				lcddev.height=480;
			}
	...
	{	  				
		...
		lcddev.width=320;
		lcddev.height=240;
		if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0x7789||lcddev.id==0x9486||lcddev.id==0x9488||lcddev.id==0x7796)
		{
			lcddev.wramcmd=0X2C;
	 		lcddev.setxcmd=0X2A;
			lcddev.setycmd=0X2B;  	 
...
{     
	...
	twidth=sx+width-1;
	theight=sy+height-1;
	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X6804 ||lcddev.id==0x9481 ||lcddev.id==0x7789 ||lcddev.id==0x9486||lcddev.id==0x9488||lcddev.id==0x7796 ||(lcddev.dir==1&&lcddev.id==0X1963))
	{
		LCD_WR_REG(lcddev.setxcmd); 
		LCD_WR_DATA(sx>>8); 
		LCD_WR_DATA(sx&0XFF);	 
...
{ 	  
	...
	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963||lcddev.id==0X7789 ||lcddev.id==0X9486||lcddev.id==0X9488 || lcddev.id == 0x7796)//如果是这几个IC,则设置WR时序为最快
	{
		//重新配置写时序控制寄存器的时序   	 							    
	...
	else if(lcddev.id==0x9481) //9481初始化
	{
//************* 新版本的9481驱动 IPS高清屏 ******************************//
		LCD_WR_REG(0x11);
		delay_ms(20);
	...
	else if(lcddev.id==0x9486) //9486初始化
	{
		LCD_WR_REG(0XF2);				
		LCD_WR_DATA(0x18);				
		LCD_WR_DATA(0xA3);				
	...
	else if(lcddev.id==0x9488) //9488初始化
	{
	...
	else if(lcddev.id==0x7796) //7796初始化
	{
		LCD_WR_REG(0x11);
		delay_ms(120); 
...
void LCD_Clear(u32 color)
{
	...
	if((lcddev.id==0X6804||lcddev.id==0X9481)&&(lcddev.dir==1))//6804横屏的时候特殊处理  
	{						    
 		lcddev.dir=0;	 
 		lcddev.setxcmd=0X2A;
		lcddev.setycmd=0X2B;  	 			
